...
s_y = np.sqrt((np.power((y/d), 2)*np.power(s_d,2)) + (np.power(d*np.cos(angoli_rad), 2)*(np.power(0.0002,2))))

m, q, s_m, s_q = fit_lineare_doro(ordini, y, s_y)

lunghezza_donda = m
lunghezze_donda = y / ordini
s_lunghezze_donda = np.sqrt((np.power((np.sin(angoli_rad)/ordini), 2)*np.power(s_d,2)) + (np.power((d*np.cos(angoli_rad)/ordini), 2)*(np.power(0.0002,2))))
